...
class BoardStack():
    ...
    def getScore(self):
        return self.jokers * 50 + self.twos * 20 + self.count * getCardCount(self.cardType)
    ...
    def __str__(self):
        return "(" + str(self.cardType) + ", " + str(self.count) + " count, " + str(self.jokers) + " jokers, " + str(self.twos) + " twos)"
...
class Game:
    ...
    def init_game(self):
        self.reset()
        return (nodesConversion(self.players[0].hand,self.players[0].board,self.discardPile,self.drawn,self.players[0]),0)
    ...
    def get_state(self,player_id):
        return nodesConversion(self.players[player_id].hand,self.players[player_id].board,self.discardPile,self.drawn,self.players[player_id])

    def step(self,action):
        executeMove(self.players[self.get_player_id()],self,action)
        return (nodesConversion(self.players[self.turn % 6].hand,self.players[self.turn % 6].board,self.discardPile,self.drawn,self.players[self.turn % 6]),self.turn % 6)
...
def checkLegal(player: Player, game, move):
    ...
    if move == "goOut":
        ...
        if game.drawn == False or handSize == 1 or (handSize == 2 and len(player.board.canastas) < 2):
            return False
        playedCard = int(move)
        if player.hand[playedCard] == 0:
            return False
        for pile in player.board.piles:
            if pile.cardType == playedCard:
                return True
        if player.hand[playedCard] >= 3 or (player.hand[playedCard] == 2 and player.hand[1] + player.hand[2] >= 1):
